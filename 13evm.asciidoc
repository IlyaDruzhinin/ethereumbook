...
BLOCKHASH  //Get the hash of one of the 256 most recently completed
           //blocks
COINBASE   // Получить адрес пользователя, который получит награду за блок
...
Когда транзакция приводит к выполнению кода смарт-контракта,  создается экземпляр EVM со всей информацией, необходимой в отношении текущего создаваемого блока и конкретной обрабатываемой транзакции. 
В частности, в ПЗУ программного кода EVM загружается код вызываемого контракта, счетчик программ обнуляется, хранилище загружается из хранилища контракта, 
в памяти установлены все нули и весь блок и переменные среды установлены. Ключевой переменной является подача газа для этого выполнения, которая устанавливается на количество газа, 
оплаченного отправителем в начале транзакции (подробнее см. «Газ»). По мере выполнения кода подача газа уменьшается в соответствии со стоимостью газа выполняемых операций. 
Если в какой-то момент подача газа снижается до нуля, мы получаем исключение «Нет газа» (OOG); выполнение немедленно останавливается и транзакция прекращается. Изменения в состоянии Ethereum не применяются, 
за исключением того, что увеличивается одноразовый номер отправителя и снижается его эфирный баланс, чтобы заплатить бенефициару блока за ресурсы, использованные для выполнения кода, до точки остановки. 
В этот момент вы можете подумать о том, что EVM работает на изолированной копии мирового состояния Ethereum, с этой изолированной версией полностью отбрасывается, если выполнение не может быть завершено по какой-либо причине. 
Однако, если выполнение завершается успешно, то реальное состояние обновляется, чтобы соответствовать изолированной версии, включая любые изменения в данных хранилища вызываемого контракта, 
любые новые созданные контракты и любые инициированные переносы баланса эфира.
