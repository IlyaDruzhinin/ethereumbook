...
* Набор консенсусных правил, governing what constitutes a transaction and what makes for a valid state transition
* Машина стостояний, который обрабатывает транзакций согласно консенсусным правилам
...
P2P сеть:: Ethereum runs on the _Ethereum main network_, which is addressable on TCP port 30303, and runs a protocol called _ÐΞVp2p_.

Консенсусные правила:: Консенсные правила Ethereum'а определны в референсной спецификации, т.н. Yellow Paper (смотри <<references>>).
...
Алгоритм консенсуса:: Ethereum использует консенсусную модель Bitcoin'а, Nakamoto консенсус, 
который использует последовательнонсть блоков с единственной подписью, взвешенные по важности PoW для определения самой длинной цепочки и, 
следовательно, текущего состояния.
Однако в ближайшем будущем планируется перейти на взвешенную систему голосования PoS под кодовым названием _Casper_.
