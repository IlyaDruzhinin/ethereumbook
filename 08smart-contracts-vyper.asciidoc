...
В то время как умные контракты могут записывать в цепочку данных Ethereum (посредством событий журнала), 
они не могут прочитать созданные в журнале события. Несмотря на это, 
одним из преимуществ записи данных цепочки Ethereum с помощью событий журнала является то, 
что журналы могут быть обнаружены и прочитаны в общедоступной цепочке легкими клиентами. Например, значение +logsBloom+ 
в добытом блоке может указывать, присутствует ли событие журнала. 
Когда существование событий журнала установлено, данные журнала могут быть получены из квитанции транзакции.
