...
((("transactions","basic structure")))Сначала давайте рассмотрим базовую структуру транзакции, так как она сериализуется и передается по сети Ethereum. 
Каждый клиент и приложение, которые получают сериализованную транзакцию, будут хранить ее в памяти, используя свою собственную внутреннюю структуру данных, 
возможно, украшенную метаданными, которых нет в самой сериализованной транзакции сети. Сериализация сети является единственной стандартной формой транзакции.

Транзакция это сериализированное двоичное сообщение, которое содержит следующие данные:

Nonce:: A sequence number, issued by the originating EOA, used to prevent message replay

Gas price (стоимость газа):: The price of gas (in wei) the originator is willing to pay

Gas limit (лимит газа):: The maximum amount of gas the originator is willing to buy for this transaction

Recipient :: Ethereum адрес назначения

Value:: Количество эфиров отправленные адресу назначения.

Data:: Переменной длины двоичные данные с полезной нагрузкой (payload).

v,r,s:: Три компонента цифровой подписи ECDSA исходного EOA

((("Рекурсивный префикс длины (RLP)")))((("RLP (Рекурсивный префикс длины)")))
Структура сообщения транзакции сериализуется с использованием схемы кодирования рекурсивного префикса длины (RLP), 
которая была создана специально для простой, идеальной для байтов сериализации данных в Ethereum. 
Все числа в Ethereum кодируются как целые числа с прямым порядком байтов, длина которых кратна 8 битам.

Обратите внимание, что метки полей (от +to+, +gas limit+ и т. Д.) показаны здесь для ясности, но не являются частью сериализованных данных транзакции, 
которые содержат значения полей, кодированные RLP. В целом, RLP не содержит разделителей или меток полей. 
Префикс длины RLP используется для определения длины каждого поля. Все, что находится за пределами определенной длины, относится к следующему полю в структуре.

Хотя это фактическая передаваемая структура транзакции, большинство внутренних представлений 
и визуализаций пользовательского интерфейса украшают ее дополнительной информацией, полученной из транзакции или из цепочки блоков.

Например, вы можете заметить, что нет &#x201c;from&#x201d; данные в адресе, идентифицирующем отправителя EOA. 
Это потому, что открытый ключ EOA может быть получен из компонентов +v,r,s+ ECDSA подписи. 
Адрес, в свою очередь, может быть получен из открытого ключа. Когда вы видите транзакцию, показывающую поле "from", 
оно было добавлено программным обеспечением, используемым для визуализации транзакции. Другие метаданные, 
часто добавляемые в транзакцию клиентским программным обеспечением, включают номер блока (после того, как он добыт и включен в цепочку блоков) 
и идентификатор транзакции (вычисленный хэш). Опять же, эти данные получены из транзакции и не являются частью самого сообщения транзакции.
