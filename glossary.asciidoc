...
Ethash::
((("Ethash"))) Алгоритм доказательства работы для Ethereum 1.0. Подробная информация - https://github.com/ethereum/wiki/wiki/Ethash.
...
Дерево Merkle Patricia::
((("Дерево Merkle Patricia")))Структура данных, используемая в Ethereum для эффективного хранения пар ключ&#x2013;значение.
...
((("proof of work (PoW)","defined")))Часть данных (доказательство), которая требует значительных вычислений, чтобы их найти. В Ethereum майнеры должны найти числовое решение алгоритма Ethash, которое соответствует общесетевому уровню сложности.
